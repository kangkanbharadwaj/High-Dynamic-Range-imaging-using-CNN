...
train_dir="/misc/lmbraid19/cicek/laura_frog_embryos/training/";
...
input_files="/misc/lmbraid11/alis/2d_dense/test_files";

NETNAME="2d_dense";
snapshot="2d_dense_snapshot_iter_350000.caffemodel.h5";

if [ -e ${snapshotdir}/${snapshot} ]; then
    sed -e "s+@scriptname@+${0}+g" \
	-e "s+@train_file@+${train_file}+g" \
	-e "s+@train_dir@+${train_dir}+g" \
...
test_files=textread('@input_files@','%s', 'delimiter','\n','whitespace','');
train_dir= '@train_dir@'
...
for file=1:length(test_files),
    ...
    fname = strrep(current_file,train_dir,sprintf(''));
    data = ncread(current_file,'data');
    current_file, size(data)
    %data = ncread('@input_data@','data');
    ...
    for p=1:1,% subject
        ...
        for dd = 1:numel(d),
            ...
            iter = str2num(d(dd).name(lastunderscore+1:end-14));
            snapshotd='@snapshotdir@';
            sprintf(fullfile(snapshotd,'%d_prediction_%s'),iter,fname)
            predfile = dir(sprintf(fullfile(snapshotd,'%d_prediction_%s'),iter,fname));
        ...
        % 'did it reach this point'
            ...
            h5outputfile = sprintf(fullfile(snapshotd,'%d_prediction_%s'),iter,fname);
